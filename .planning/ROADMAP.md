...
**Success Criteria** (what must be TRUE):
  ...
  2. Moving the frame slider scales the `Konva.Image` frame node while keeping it centered in the stage
  3. Aspect ratio is preserved for both nodes (uniform scale via `scaleX` = `scaleY`)
  4. Stage redraws in real-time on every slider `input` event (no lag, no manual `layer.draw()` needed)
**Plans**: TBD
**UI hint**: yes

**Suggested plan breakdown (1-3 plans for coarse):**
  - Plan 3.1: `photoScale` and `frameScale` slider inputs; on change, recalculate `x/y` offset to keep node centered and set `scaleX`/`scaleY`; call `layer.batchDraw()`
